...
l<-3
for(l in 4:4){
order1<-c(14,11,9,10,12,13,15,16)
pctwets<-c('dam',0,0.1,0.2,1,5,10,25)
#data2<-read.csv('distribution/raw/yearsoutput_loop_0.2.csv',head=FALSE)
data2<-read.csv(paste(path,'raw/',files[order1[l]],sep=""),head=FALSE)
...
points(species_dist$longitude, species_dist$latitude, col="black", cex=0.05,pch=16)
species<-"Tiliqua occipitalis"
species_dist=occurrences(taxon=species, download_reason_id=10)
species_dist<-as.data.frame(species_dist$data)
points(species_dist$longitude, species_dist$latitude, col="blue", cex=0.05,pch=16)
...
species_dist=occurrences(taxon=species, download_reason_id=10)
species_dist<-as.data.frame(species_dist$data)
points(species_dist$longitude, species_dist$latitude, col="red", cex=0.05,pch=16)
