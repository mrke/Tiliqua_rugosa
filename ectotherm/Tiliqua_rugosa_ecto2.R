...
#DEP <- c(0., 2.5,  5, 10,  15,  20,  30.,  60.,  100.,  200.) # Soil nodes (cm) - keep spacing close near the surface, last value is where it is assumed that the soil temperature is at the annual mean air temperature
  ...
  micro<-micro_aust(loc = longlat, ystart = ystart, yfinish = yfinish, PE = PE, BB = BB, BD = 
      BD, KS = KS, BulkDensity = BulkDensity, maxshade = 90, Usrhyt = 3, DEP = DEP, REFL = 0.2)
...
nicheout=ectotherm(ABSMAX = 0.866, ABSMIN = 0.866, VTMAX = 39, VTMIN = 26, write_input = 0, EMISAN = 1, gutfill = 90,
  TBASK = TBASK, TEMERGE = 8.5, ctmax = 39, ctmin = 3.5, TPREF = 33.5,  peyes = 0.03, ptcond = 0.1, skinwet = 0.2,
  shdburrow = 0, minwater = 15, mindepth = 3, raindrink = 2.5, DEB = 1, z=debpars[8], del_M=debpars[9], F_m = 13290,  
  ...
  E_sm=186.03*6, K = 500, X = 11.7, plantsim = c(4, 4, 14, -200, -1500, 82), clutch_ab = c(0.085,0.7), viviparous = 1, photostart = 3, 
  E_init = ((debpars[16]/24)*debpars[8]/debpars[14])/(debpars[13]/24), E_H_init = debpars[20]+5,  
...
with(debout, {plot(WETMASS~dates,ylim=c(0,1200),type = "l",ylab = "wet mass (g)/grass",col='black')})
...
plotenviron_bask = subset(plotenviron2,  subset=(ACT>=1 & TC>=VTMIN))
plotenviron_forage = subset(plotenviron2,  subset=(ACT>1))
plotenviron_trap = subset(plotenviron2,  subset=(ACT>=1 & SHADE==0 & TC>=VTMIN))
...
#points(foodwaters*100~dates2,type='l',col='green')
...
grass=as.numeric(foodwaters[,1])
points(grass*2+100~dates,type='l',col='dark green')
...
plotenviron_bask = subset(plotenviron,  subset=(ACT>=1 & TC>=VTMIN))
plotenviron_forage = subset(plotenviron,  subset=(ACT>1))
plotenviron_trap = subset(plotenviron,  subset=(ACT>=1 & SHADE==0 & TC>=VTMIN))
